...
export const Delete=async(req:Request,res:Response,next:NextFunction)=>{
  try{
     const {id}=req.params;
     const application=await StudentRepository.getRepo().findById(id);
     if(!application){
      next(new AppError("Student to found",404,"Operational"));
     }
     const result= await StudentRepository.getRepo().Delete(id);
     if(!result){
      next(new AppError("Student not deleted",400,"Operational"));
     }
     const responseBody={status:"success",message:"Student deleted successfully"};
     res.status(200).json(responseBody);

  }catch(error){
    next(new AppError("Error occured during student",400,"Operational"))
  }
}