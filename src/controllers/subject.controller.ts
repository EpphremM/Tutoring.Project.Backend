...
import {
  subjectSchema,
  subjectUpdateSchema,
} from "../zod/schemas/subject.schema";
...
export const findAll = async (
  ...
  const responseBody: ResponseBody<SubjectInterface[]> = {
    status: "success",
    message: "all subjects fetched sucessfully",
    data: { payload: result },
  };
  res.status(200).json(responseBody);
};
export const findById = async (
  ...
  try {
    ...
    const result: SubjectInterface = await SubjectRepository.getRepo().findById(
      id
    );
    if (!result) {
    ...
    next(
      new AppError("error occured during fetching", 400, "operational", error)
    );
  }
};
export const update = async (
  ...
  try {
    ...
    const subject: SubjectInterface =
      await SubjectRepository.getRepo().findById(id);
    if (!subject) {
      next(new AppError("subject is not found by this id", 404, "operational"));
    }
    const result: SubjectInterface = await SubjectRepository.getRepo().update(
      subject,
      body
    );
    const responseBody: ResponseBody<SubjectInterface> = {
    ...
    next(
      new AppError("error occured during updating subject", 400, "operational")
    );
  }
};
...
export const Delete = async (
  ...
  try {
    const { id } = req.params;
    const application = await SubjectRepository.getRepo().findById(id);
    if (!application) {
      next(new AppError("Subject to found", 404, "Operational"));
    }
    const result = await SubjectRepository.getRepo().Delete(id);
    if (!result) {
      next(new AppError("Subject not deleted", 400, "Operational"));
    }
    const responseBody = {
      status: "success",
    ...
    next(new AppError("Error occured during subject", 400, "Operational"));
  }
};
